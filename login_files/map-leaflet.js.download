$(document).ready(function($) {
        ...
        for (var i = 0; i < loadedMarkersData.length; i++) {
            ...
            markerContent.innerHTML =
                '<div class="ts-marker-wrapper">' +
                    ...
                    ( ( loadedMarkersData[i]["marker_image"] !== undefined ) ? '<div class="ts-marker__image ts-black-gradient" style="background-image: url(' + loadedMarkersData[i]["marker_image"] + ')"></div>' : '<div class="ts-marker__image ts-black-gradient" style="background-image: url(' + loadedMarkersData[i]["imageB"] + ')"></div>') +
                    '</a>' +
                '</div>';
        ...
        infoboxHtml.innerHTML =
            '<div class="infobox-wrapper">' +
                '<div class="ts-infobox" data-ts-id="' + loadedMarkersData[i]["id"] + '">' +
                    '<img src="/temp/assets/img/infobox-close.svg" class="ts-close">' +
                    ...
                    '<a href="' + loadedMarkersData[i]["url"] + '" class="ts-infobox__wrapper ts-black-gradient">' +
                        ...
                        '<div class="ts-infobox__content">' +
                            ...
                            '</div>' +
                        '<div class="ts-infobox_image" style="background-image: url(' + loadedMarkersData[i]["imageB"] + ')"></div>' +
                    '</a>' +
                '</div>' +
            '</div>';
    }
        ...
        for (i = 0; i < visibleMarkersOnMap.length; i++) {
            ...
            resultsHtml.push(
                '<div class="ts-result-link" data-ts-id="' + loadedMarkersData[id]["id"] + '" data-ts-ln="' + newMarkers[id].loopNumber + '">' +
                    '<span class="ts-center-marker"><img src="temp/assets/img/result-center.svg"></span>' +
                    '<a href="' + loadedMarkersData[id]["url"] + '" class="card ts-item ts-card ts-result">' +
                        ...
                        '<div href="detail-01.html" class="card-img ts-item__image" style="background-image: url(' + loadedMarkersData[i]["imageB"] + ')"></div>' +
                        '<div class="card-body">' +
    ...
    if( $("#"+simpleMapId).length ){
        ...
        var icon = L.icon({
            iconUrl: "temp/assets/img/marker-small.png",
            iconSize: [22, 29],
            iconAnchor: [11, 29]
        });
